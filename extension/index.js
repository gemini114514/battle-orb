...
try {
    ...
    settings = { writeVerdictBasis: true, recognizeHint: '', unitHint: '', fastModeling: false, rollbackModeling: false, maxRetries: 3, networkRetries: 6, debugMode: false, previewCombat: true, liteEvents: false, api: { declaration: defaultWorkApi(), modeling: defaultWorkApi() }, ...stored };
    ...
    settings.liteEvents = Boolean(settings.liteEvents);
...
async function createBattleCore() {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        battle.__combatBenchmark = { spans: {} };
        // 可选：大型尸潮战场开启“Lite 事件模式”，把海量纯噪音等待/搜索事件合并，
        // 降低账本与 DEBUG 导出体积；关键事件（命中/死亡/感知变化等）不受影响。
        battle.__liteEvents = Boolean(settings.liteEvents);
...
function bindPanel() {
    ...
    document.addEventListener('input', event => {
        ...
        if (event.target.id === 'battle-orb-lite-events') { settings.liteEvents = Boolean(event.target.checked); saveSettings(); setStatus(settings.liteEvents ? 'Lite 事件模式已开启：仅对之后创建的新战场生效' : '已关闭 Lite 事件模式', 'ok'); return; }
